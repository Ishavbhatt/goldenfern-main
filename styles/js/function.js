...
jQuery(function() {
    var top = jQuery('.room_suit_left').offset().top - parseFloat(jQuery('.room_suit_left').css('marginTop').replace(/auto/, 0));
    var footTop = jQuery('.empty_div').offset().top - parseFloat(jQuery('.empty_div').css('marginTop').replace(/auto/, 0));
    ...
    jQuery(window).scroll(function(evt) {
        ...
        if (y >= top - jQuery('#fixedHeader').height()) {
            if (y < maxY) {
                ...
                jQuery('.room_suit_left').removeClass('fixed').css({
                    position: 'absolute',
                    top: (maxY - top) + 'px'
                });
            }
